...
public:
  ...
  // Ask a realtime LLM to drop and re-establish its session (e.g. after the persona
  // role changed) so the next session.update carries the new instructions. Default
  // no-op for non-realtime LLMs. Implemented on the WebSocket task — safe to call
  // from any task (sets a flag).

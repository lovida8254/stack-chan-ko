...
// --- Custom extension: dynamic eye-radius scale (proximity "big eyes") -------
static float s_eyeRadiusScale = 1.0f;
void setEyeRadiusScale(float scale) {
  if (scale < 0.3f) scale = 0.3f;
  if (scale > 3.0f) scale = 3.0f;
  s_eyeRadiusScale = scale;
}
float getEyeRadiusScale() { return s_eyeRadiusScale; }
// -----------------------------------------------------------------------------
...
void Eye::draw(M5Canvas *spi, BoundingRect rect, DrawContext *ctx) {
  // Local `r` shadows the member for this frame, scaled by the global factor so
  // the proximity feature can grow/shrink both eyes uniformly. All the masking
  // math below references this local `r`, so masks scale with the circle.
  uint16_t r = (uint16_t)(this->r * s_eyeRadiusScale + 0.5f);

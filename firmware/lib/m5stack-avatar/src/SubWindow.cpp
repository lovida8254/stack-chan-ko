...
// SDカードのファイルを配列にコピー。
// 注意: 失敗時に SD.end() してはいけない。開いている他のハンドル(ディレクトリ等)を
// 無効化し、一度の失敗が全ファイル open 失敗へ連鎖する。リトライは呼び出し側
// (updateDrawContentJpg の 2回ループ) が担当する。
size_t SubWindow::copySDFileToRAM(const char *path, uint8_t *out, int outBufSize) {
  ...
  if (!SD.exists(path)) { Serial.println("File doesn't exist."); return 0; }

  File fsrc = SD.open(path, FILE_READ);
  if (!fsrc) { Serial.println("Failed to open file."); return 0; }

  size_t size = fsrc.size();
  if ((int)size > outBufSize) {
  ...
  if (!buf) { Serial.println("Failed to allocate buffer."); fsrc.close(); return 0; }

  uint8_t *outp = out;
  size_t len = size;
  while (len) {
    size_t s = (len > BUF_SIZE) ? BUF_SIZE : len;
    if (!fsrc.read(buf, s)) { Serial.println("Failed to read file."); delete[] buf; fsrc.close(); return 0; }
    memcpy(outp, buf, s);
    outp += s;
    len  -= s;
    delay(20);   // この delay がないと ff_sd_status が頻発する
  }
  delete[] buf;
  fsrc.close();
